	...
	public MultiSelectCombo(Composite parent, int style) {
		...
		btnMenu.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				Point control_point = ((Control) event.widget).toDisplay(1, 1 + ((Control) event.widget).getBounds().height);
				Robot robot = null;
				try {robot = new Robot();} catch (AWTException exception) {exception.printStackTrace();}
